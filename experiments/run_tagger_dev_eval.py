"""Run T5-small tagger baseline on the held-out dev_eval split.

Mirrors run_tagger_baseline.py but targets dev_eval.jsonl.
NOTE: This will take ~20 minutes on CPU (~827 sentences).

Outputs:
    results/tagger_dev_eval_preds.jsonl
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOGS_DIR / "tagger_dev_eval.log", mode="w"),
    ],
)
...
def main() -> None:
    ...
    # Run tagger
    tagger = TaggerWrapper(device="cpu", seed=SEED)
    t0 = time.time()
    results = tagger.batch_correct(sentences, log_every=200)
    elapsed = time.time() - t0

    log.info("Done in %.1f s (%.1f sent/s)", elapsed, len(sentences) / elapsed)

    # Save
    out_path = RESULTS_DIR / "tagger_dev_eval_preds.jsonl"
    ...
    print(f"\n=== Tagger Baseline Results (dev_eval) ===")
