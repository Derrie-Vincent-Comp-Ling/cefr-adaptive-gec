"""Run the GECToR tagger (or T5-small fallback) baseline on dev-tune.

Outputs:
    results/tagger_dev_tune_preds.jsonl
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOGS_DIR / "run_tagger_baseline.log", mode="w"),
    ],
)
...
def main() -> None:
    ...
    # 2) Run tagger baseline
    tagger = TaggerWrapper(device="cpu", seed=SEED)
    log.info("Running %s on %d sentences …", tagger.engine_name, len(sentences))
    t0 = time.time()
    results = tagger.batch_correct(sentences, log_every=500)
    elapsed = time.time() - t0
    log.info("Done in %.1f s (%.1f sent/s)", elapsed, len(sentences) / elapsed)

    # 3) Save predictions
    preds_path = RESULTS_DIR / "tagger_dev_tune_preds.jsonl"
    ...
    print(f"\n=== Tagger Baseline Results ({tagger.engine_name}) ===")
